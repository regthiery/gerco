 ...
 * Resolutions.php
 *
 * Classe gérant les résolutions d'une assemblée générale.
 *
 * La classe Resolutions traite des résolutions d'une assemblée générale.
 * Elle a pour but essentiel de déterminer si la résolution est adoptée ou non.
 *
 * PHP version 7
 *
 * @category Gerco
...
class Resolutions extends DataObjects
{
     ...
     * @var GeneralMeetings
     */
    protected GeneralMeetings $generalMeetings;
    ...
    public function calculateVotingResult($resolutionIndex)
    {
        ...
        if (in_array("all", $inFavour) && count($inFavour) == 1) {
            $inFavour = $presentsKeys;
        }
        if (in_array("all", $against) && count($against) == 1) {
            $against = $presentsKeys;
        }
        if (in_array("all", $abstention) && count($abstention) == 1) {
            $abstention = $presentsKeys;
        }


        if (in_array("none", $inFavour) && count($inFavour) == 1) {
            $inFavour = array();
        }
        if (in_array("none", $against) && count($against) == 1) {
            $against = array();
        }
        if (in_array("none", $abstention) && count($abstention) == 1) {
            $abstention = array();
        }
        ...
        uasort($unrepresented, function ($a, $b) {
            return strcmp($a["pseudo"], $b["pseudo"]);
        });
        uasort($yes, function ($a, $b) {
            return strcmp($a["pseudo"], $b["pseudo"]);
        });
        uasort($no, function ($a, $b) {
            return strcmp($a["pseudo"], $b["pseudo"]);
        });
        uasort($abs, function ($a, $b) {
            return strcmp($a["pseudo"], $b["pseudo"]);
        });
    ...
    public function displayVotingResult($resolutionIndex)
    {
        ...
        foreach ($resolution["yes"] as $pseudo => $ownerData) {
            printf(
                "\t%-20s\t%-20s\t%10d/%-7d (%5.2f%%) \t %10d/%-7d (%5.2f%%)\n",
                $ownerData["lastname"],
                $ownerData["firstname"],
                $ownerData["value"],
        ...
        printf(
            "Total\t%-20s\t%-20s\t%10d/%-7d \033[1m(%5.2f%%)\033[0m \t %10d/%-7d \033[1m(%5.2f%%)\033[0m\n",
            "--",
            "--",
            $resolution["sumYes"],
            $resolution["sumRegistered"],
        ...
        printf(
            "Total\t%-20s\t%-20s\t%10d/%-7d \033[1m(%5.2f%%)\033[0m \n",
            "--",
            "--",
            $resolution["sumAbs"],
            $resolution["sumRegistered"],
        ...
        print("\n\n");
        printf(
            "\033[1mInscrits    \033[0m\t%5.0f tantièmes\n",
            $resolution["sumRegistered"]
        );
        printf(
            "\033[1mReprésentés \033[0m\t%5.0f tantièmes \t %5.2f %% des inscrits\n",
            $resolution["sumRepresented"],
            $resolution["ratioRepreRegis"]
        );
        printf(
            "\033[1mExprimés  \033[0m\t%5.0f tantièmes \t %5.2f %% des inscrits\n",
            $resolution["sumExprims"],
            $resolution["ratioExpriRegis"]
        );
        printf(
            "\033[1mAbstention \033[0m\t%5.0f tantièmes \t %5.2f %% des inscrits\n",
            $resolution["sumAbs"],
            $resolution["ratioAbsRegis"]
        );
        printf(
            "\033[1mPour      \033[0m\t%5.0f tantièmes \t %5.2f %% des inscrits\t %5.2f %% des exprimés\n",
            $resolution["sumYes"],
            $resolution["ratioYesRegis"],
            $resolution["ratioYesExprim"]
        );
        printf(
            "\033[1mContre     \033[0m\t%5.0f tantièmes \t %5.2f %% des inscrits\t %5.2f %% des exprimés\n",
            $resolution["sumNo"],
            $resolution["ratioNoRegis"],
            $resolution["ratioNoExprim"]
        );
        ...
        if ($resolution["majority"] == 26) {
            ...
            printf(
                "\t Voix pour   : %6d tantièmes \t %.2f %%\n",
                $resolution["sumYes"],
                $resolution["ratioYesExprim"]
            );
            printf(
                "\t Voix contre : %6d tantièmes \t %.2f %%\n",
                $resolution["sumNo"],
                $resolution["ratioNoExprim"]
            );
        }
