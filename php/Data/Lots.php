 ...
 * Lots.php
 *
 * Classe listant les lots de la copropriété
 *
 * La classe Lots stocke l'ensemble des informations sur les lots constituant la copropriété.
 *
 * PHP version 7
 *
 * @category Gerco
...
class Lots extends DataObjects
{
    ...
    /**
     * Setter function pour la propriété imputations
     *
     * @param $imputations
     */
            ...
            if ($item['type'][0] == 'T') {
                foreach ($this->imputations->getObjects() as $imputationKey => $imputationData) {
                    if (array_key_exists($imputationKey, $this->objects[$key])) {
                        $this->objects[$key]['imputations'][$imputationKey]
                            = $this->objects[$key][$imputationKey];
                    }
                }
    ...
    public function showPrices()
    {
        ...
        $this->logger->displayData(
            ...
            "situation>6",
            "general>12",
            "imputations:general>12",
            "ownerData:lastname>15"
        );
    }
    ...
    public function showOwnersByBatiment($batiment)
    {
        ...
        $this->filteredObjects = array_filter(
        ...
        $this->logger->displayData(
            ...
            "floor>6",
            "situation>6",
            "imputations:general>9",
            "imputations:special$batiment>9",
            "ownerData:lastname>18"
        );
        $this->logger->displaySums(
            "imputations:general",
            "imputations:special$batiment"
        );
    }
}
